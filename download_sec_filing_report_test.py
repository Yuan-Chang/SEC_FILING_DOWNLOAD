...
def get_report_summary(wb: Workbook, cik: str, company_name: str):
    ...
    for column_index, qDict in enumerate(operatingIncomeLossList):
        ...
        values = [q.fp, q.frame, q.start, q.end, toMillion(q.operatingIncomeLoss), toMillion(q.incomeTaxExpenseBenefit),
                  toMillion(q.incomeBeforeIncomeTax), toMillion(q.incomeBeforeIncomeTaxAndEquityInterest)]
